...
class ControlLoRA_CLIP(ModelMixin, ConfigMixin):
    @register_to_config
    def __init__(
        self,
        in_channels: int = 3,
        ...
        self.conv_in = torch.nn.Conv2d(in_channels + 1, block_out_channels[0], kernel_size=3, stride=1, padding=1)
        ...
        # self.cross_attn_in = CrossAttention(query_dim=512, )
        
    def forward(self, x: torch.FloatTensor, prompt, return_dict: bool = True) -> Union[ControlLoRAOutput, Tuple]:
        lora_layer: ControlLoRACrossAttnProcessor
        ...
        # h = x.to(dtype) # B 3 512 512
        # h = self.conv_in(h) # B 32 512 512
        h = x.to(dtype) # B 3 512 512
        
        # with torch.no_grad():
        #     text_token = self.tokenizer(prompt, padding=True, return_tensors="pt")
        #     text_embedding = self.text_model(**text_token).pooler_output #B 512
        #     vision_embedding = self.vision_model(**self.processor(images=x, return_tensors="pt")).pooler_output #B 768
        seg_input = self.tokenizer(prompt, padding=True, return_tensors="pt")
        for k, v in seg_input.items():
            seg_input[k] = v.to("cuda:1")
        seg_input["pixel_values"] = x.to("cuda:1")
        # with torch.no_grad():
        self.clipseg_model.to("cuda:1")
        outputs = self.clipseg_model(**seg_input)
        target_logits = torch.sigmoid(outputs.logits).to(device)
        # target_logits = torch.nn.functional.interpolate(target_logits, size=x.shape[-2:], mode="nearest")
        h = torch.cat([h, target_logits.reshape(h.shape[0], 1, h.shape[2], h.shape[3]).to(dtype)], dim=1) # conv_in channel add 1
        h = self.conv_in(h)
