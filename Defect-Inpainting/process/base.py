...
class Dataset(data.Dataset):
    ...
    @staticmethod
    def cat_input(Generated_image:Image.Image,image: Image.Image, target: torch.Tensor, guide: torch.Tensor):
        ...
        image_cat = Image.new('RGB', (image.size[0]*4,image.size[1]), (0,0,0))
        image_cat.paste(target,(0,0))
        image_cat.paste(guide,(image.size[0], 0))
        image_cat.paste(image,(image.size[0]*2, 0))
        image_cat.paste(Generated_image,(image.size[0]*3, 0))
