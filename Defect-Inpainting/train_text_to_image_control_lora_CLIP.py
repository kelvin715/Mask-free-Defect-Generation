...
def parse_args():
    ...
    parser.add_argument(
        "--validation_prompt", type=str, default="An image with 2 scratch defect on the metal_nut", help="A prompt that is sampled during training for inference."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="/Data4/student_zhihan_data/Diffusion/ControlLoRA/output/sd-mvtec-model-control-lora",
    ...
    parser.add_argument("--item_type", type=str, default="capsule", help="The type of item to train on.")
    parser.add_argument(
        "--hub_model_id",
        type=str,
        default=None,
    ...
    parser.add_argument(
        "--logging_dir",
        type=str,
        default="/Data4/student_zhihan_data/Diffusion/ControlLoRA/logs_mvtec",
        # default=None,
        help=(
...
def main():
    ...
    if use_custom_dataset:
        ...
        def preprocess_train(examples):
            images, guides, prompts = [], [], []
            for image, guide, prompt in zip(examples[image_column], examples[guide_column], examples[caption_column]):
        ...
        with accelerator.main_process_first():
            dataset["train"] = dataset["train"].filter(lambda x: args.item_type in x["text"])
        ...
        def collate_fn(examples):
            ...
            return {"pixel_values": pixel_values, "guide_values": guide_values, "input_ids": input_ids, "prompts": prompts}
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                _ = control_lora(batch["guide_values"].to(dtype=weight_dtype), batch["prompts"]).control_states
                        ...
                        if args.validation_prompt is not None:
                            ...
                            for _ in range(args.num_validation_images):
                                with torch.no_grad():
                                    ...
                                    _ = control_lora(guide, batch["prompts"]).control_states
                                    image = pipeline(
