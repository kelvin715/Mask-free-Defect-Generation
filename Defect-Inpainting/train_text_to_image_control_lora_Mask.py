...
from diffusers.models import attention as cross_attention
...
# from models import ControlLoRA_CLIP as ControlLoRA
...
def parse_args():
    ...
    parser.add_argument(
        "--validation_prompt", type=str, default="Uneven", help="A prompt that is sampled during training for inference."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="/Data4/student_zhihan_data/Diffusion/ControlLoRA/output/sd-magnetic-model-control-lora",
    ...
    parser.add_argument(
        "--logging_dir",
        type=str,
        default="/Data4/student_zhihan_data/Diffusion/ControlLoRA/logs",
        # default=None,
        help=(
    ...
    parser.add_argument("--control_lora_config", type=str, default="/Data4/student_zhihan_data/Diffusion/ControlLoRA/configs/fill50k.json", help="Config file of ControlLora")
...
DATASET_NAME_MAPPING = {
    "Kelvin878/magnetic": ("image", "guide", "text"),
...
def main():
    ...
    if use_custom_dataset:
        ...
        def preprocess_train(examples):
            ...
            for image, guide in zip(examples[image_column], examples[guide_column]):
                image, guide = image.convert("RGB"), guide.convert("L")
                #convert guide to binary with single channel
                image, guide = train_transforms(image), binary_transforms(guide)
                c, h, w = image.shape
                y1, x1 = 0, 0
                ...
                background[(guide != 0).repeat(3, 1, 1)] = 0
                masked_image = image * background
                #concat image and guide
                # guide = torch.cat((image, guide), dim=0)
                images.append(image)
                guides.append(guide)
        ...
        def collate_fn(examples):
            ...
            masked_values = torch.stack([example["masked_images"] for example in examples])
            return {"pixel_values": pixel_values, "guide_values": guide_values, "input_ids": input_ids, "prompts": prompts, "masked_values": masked_values}
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                encoder_hidden_states = text_encoder(batch["input_ids"].to("cuda:1"))[0].to("cuda:0")

                # Inject control states to unet
                _ = control_lora(torch.concat([batch["guide_values"].to(dtype=weight_dtype), batch["masked_values"].to(dtype=weight_dtype)], dim=1)).control_states
                ...
                if global_step % args.checkpointing_steps == 0:
                    if accelerator.is_main_process:
                        ...
                        if args.validation_prompt is not None:
                            ...
                            inpaint_pipeline.unet = accelerator.unwrap_model(unet)
                            ...
                            for _ in range(args.num_validation_images):
                                with torch.no_grad():
                                    try:
                                        ...
                                        val_iter = iter(val_dataloader)
                                        batch = next(val_iter)
                                    target = batch["pixel_values"].to(accelerator.device)
                                    guide = batch["guide_values"].to(accelerator.device)
                                    masked = batch["masked_values"].to(accelerator.device)
                                    _ = control_lora(torch.concat([guide, masked], dim=1)).control_states
                                    image = pipeline(
                                        batch["prompts"][0], num_inference_steps=30, generator=generator).images[0]
                                    inpainted_image = inpaint_pipeline(prompt=batch["prompts"][0], image=target, mask_image=guide, num_inference_steps=30, generator=generator).images[0]
                                    image = dataset_cls.cat_input(inpainted_image, image, target, masked)
                                images.append(image)
            ...
            if args.validation_prompt is not None and epoch % args.validation_epochs == 0:
                ...
                for _ in range(args.num_validation_images):
                    with torch.no_grad():
                        ...
                        masked = batch["masked_values"].to(accelerator.device)
                        _ = control_lora(torch.concat([guide, masked], dim=1)).control_states
                        image = pipeline(batch["prompts"][0], num_inference_steps=30, generator=generator).images[0]
                        inpainted_image = inpaint_pipeline(prompt=batch["prompts"][0], image=target, mask_image=guide, num_inference_steps=30, generator=generator).images[0]
                        image = dataset_cls.cat_input(inpainted_image, image, target, masked)
    ...
    for _ in range(args.num_validation_images):
        with torch.no_grad():
            ...
            masked = batch["masked_values"].to(accelerator.device)
            _ = control_lora(torch.concat([guide, masked], dim=1)).control_states
            image = pipeline(prompt=batch["prompts"][0], num_inference_steps=30, generator=generator).images[0]
            inpainted_image = inpaint_pipeline(prompt=batch["prompts"][0], image=target, mask_image=guide, num_inference_steps=30, generator=generator).images[0]
            image = dataset_cls.cat_input(inpainted_image, image, target, masked)
        images.append(image)
